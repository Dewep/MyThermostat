...
async function refreshWatcherAction () {
  ...
  const programs = state.programs.filter(p => p.enabled)
  if (!state.manualUntil && programs.length) {
    const currentProgram = findProgramForDate(programs, now, false)
    ...
    const nextProgram = findProgramForDate(programs, now, true)
...
async function refreshState (edition) {
  ...
  const program = findProgramForDate(state.programs.filter(p => p.enabled), dateFrom, true)
