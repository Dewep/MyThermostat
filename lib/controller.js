...
async function refreshWatcherAction () {
  ...
  console.info('[watcher-temperature]', state.setTemperature, 'vs', state.wantedTemperature)

  if (state.setTemperature !== state.wantedTemperature) {
    if (state.setTemperature < state.wantedTemperature) {
      await eWeLink.increase()
      await updateStateSetTemperature(1)
      console.info('[set-temperature] +0.1')
    } else {
  ...
  console.info('timerBeforeChange', timerBeforeChange, 's')
